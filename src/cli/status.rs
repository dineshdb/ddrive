...
impl<'a> StatusCommand<'a> {
    ...
    async fn gather_stats(&self) -> Result<RepositoryStats> {
        ...
        let (new_files_refs, changed_files, deleted_files, renames) = processor
            .detect_changes_lightweight(&all_files, &tracked_file_records)
            .await?;

        // Convert to string paths for display
        let new_files: Vec<String> = new_files_refs
            .iter()
        ...
        let total_untracked_size: u64 = new_files_refs.iter().map(|f| f.size).sum();
